def validate_number_params_not_negative(function_name, **args):
    for param_name, value in args.items():
        if value is None:
            ...
            raise TypeError(f"Invalid type for parameter '{param_name}' in function '{function_name}'. Expected a number.")
        elif value < 0:
...
def validate_object_params_not_none(function_name, **args):
    for param_name, value in args.items():
        if value is None:
            raise TypeError(f"Invalid 'None' value for parameter '{param_name}' in function '{function_name}'.")
