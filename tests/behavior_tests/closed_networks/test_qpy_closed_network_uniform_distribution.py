...
LOWER_BOUND_TERMINAL = 0.5
UPPER_BOUND_TERMINAL = 2.5

LAMBDA = 1/((LOWER_BOUND_TERMINAL + UPPER_BOUND_TERMINAL) / 2)
MU = 1/((LOWER_BOUND_SERVER + UPPER_BOUND_SERVER) / 2)

def _create_environment_with_servers(num_of_servers: int) -> Environment:
    env = Environment(number_of_terminals=TERMINALS, think_time_distribution=Distribution.uniform(LOWER_BOUND_TERMINAL, UPPER_BOUND_TERMINAL))
...
def test_closed_network_MM1_uniform_FCFS():
    ...
    expected_number_of_jobs = TOTAL_SIMULATION_TIME * MU
    ...
    assert value_within_range(MU, ans.environment_metrics.get_mean_time_in_system() - ans.environment_metrics.get_mean_queue_time(), DELTA_JOBS_PERCENTAGE)

def test_closed_network_MM2_all_jobs_go_through_both_servers_uniform_FCFS():
...
def test_closed_network_MM3_all_jobs_go_through_server_0_and_half_chance_to_both_servers_uniform():
    ...
    expected_utilization = 0.8 # Lower because of other server
    expected_queue_time = 1
    expected_time_in_system = expected_queue_time + 2 * 1/MU # Two servers
    expected_number_of_jobs = TOTAL_SIMULATION_TIME * TERMINALS * 10 / (expected_time_in_system + THINK_TIME)

    assert value_within_range(expected_number_of_jobs, ans.environment_metrics.get_number_of_processed_jobs(), DELTA_JOBS_PERCENTAGE)
    assert value_within_range(expected_number_of_jobs / 2, ans.server_metrics[1].get_number_of_processed_jobs(), DELTA_JOBS_PERCENTAGE)
    assert value_within_range(expected_number_of_jobs / 2, ans.server_metrics[2].get_number_of_processed_jobs(), DELTA_JOBS_PERCENTAGE)
    assert value_within_range(expected_number_of_jobs, ans.environment_metrics.get_number_of_processed_jobs(), DELTA_JOBS_PERCENTAGE)
    assert value_within_range(expected_utilization, ans.server_metrics[0].get_server_utilization(), DELTA_JOBS_PERCENTAGE)
    ...
    assert value_within_range(MU * 2, ans.environment_metrics.get_mean_time_in_system() - ans.environment_metrics.get_mean_queue_time(), DELTA_JOBS_PERCENTAGE)